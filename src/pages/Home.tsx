...
function Home() {
  ...
  const { error, data, isLoading, refetch } = useNavidromeRequest<SubsonicResponse>(
    ...
    <section className="space-y-6">
      <div className="glass-panel rounded-2xl p-5 sm:p-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Authenticated as {credentials.username}. Fetch available albums from Navidrome.
        </p>
        ...
        {!error && albums.length > 0 ? (
          <p className="mt-3 text-sm text-primary">Loaded {albumCount}</p>
        ) : null}
