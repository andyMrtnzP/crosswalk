...
import type { AlbumList2Response, AlbumRecord, ArtistRecord, ArtistsResponse, Playlist, PlaylistsResponse } from '@/@types/types';
...
export default function Library() {
  ...
  const artists = 
    (artistsData?.['subsonic-response']?.artists?.index ?? []).flatMap((group) => group.artist ?? []).slice(0, 6)
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-6 px-9 pt-9 pb-6">
        ...
        <div className="text-[12px] tracking-[0.04em] tabular-nums text-ink-3">
          <span className="text-ink-2">
            {playlists.length + artists.length} {playlists.length + artists.length === 1 ? 'item' : 'items'}
          </span>
          {' · '}
